 ...
 *                           Revision History
 * ****************************************************************************
 * Feb 07, 2026 - Narciso M: Completed implementation of findEnd() method.
 * Feb 06, 2026 - Narciso M: Implemented the findFront() method.
 * Feb 03, 2026 - Narciso M: Updated documentation and formatting.
 * Feb 01, 2026 - Narciso M: Initial code review and setup.
 * ****************************************************************************
 */
package student;
...
public class RaggedArrayList<E> implements Iterable<E> {
    ...
    public ListLoc findFront(E item) {
        int l1 = 0, l2 = 0;  // Start searching from the beginning

    for (; l1 < l1NumUsed; l1++) {
        L2Array l2Array = (L2Array) l1Array[l1];

        // Find the first match or the first greater element
        for (l2 = 0; l2 < l2Array.numUsed; l2++) {
    ...
    for (; l1 < l1NumUsed; l1++) {
        ...
        // Scan through L2 array
        for (l2 = 0; l2 < l2Array.numUsed; l2++) {
            if (comp.compare(l2Array.items[l2], item) > 0) {  // First greater item
                return new ListLoc(l1, l2);
            }
        }
    }
